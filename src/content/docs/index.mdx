...
In its basic form it's a plain SQLite loadable extension, so any language that can `SELECT load_extension('honker_ext')` gets the same queue, streams, and notifications on the same file. Bindings for Python, Node, Rust, Go, Ruby, Bun, Elixir, C++, .NET / C#, Java, and Kotlin share one on-disk format.
