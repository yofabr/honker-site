...
tx.query(
  ...
  ['greetings', JSON.stringify({ name: 'world' }), null, null, 0, 3, null],
);
tx.commit();
...
const jobs = JSON.parse(
  rtx.query(
    ...
    ['greetings', 'worker-1', 1, 300],
  )[0].j,
);
rtx.commit();

const job = jobs[0];
console.log(`hello, ${JSON.parse(job.payload).name}!`);
...
tx.query(
  "SELECT honker_enqueue(?, ?, ?, ?, ?, ?, ?)",
  ['emails', JSON.stringify({ order_id: 42, email_type: 'confirmation' }),
   null, null, 0, 3, null],
);
tx.commit();
```
